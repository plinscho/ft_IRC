...
void Server::initServer() {
    ...
    // This means that calls to send, recv, accept, etc. on this socket will
    // return immediately rather than waiting for the operation to complete.
    ...
    //	configurar el socket para que reutilice la dirección y el puerto cuando
    // se cierre.
    int opt = 1;
    ...
    // 	The bind() function assigns a IP address to a socket & a port number in
    // the local machine.
    //	The data is specified in "_sockaddr", where one field is the IP address
    // and the other is the port number.
    ...
    initPoll();
    //  listen() marks the socket referred to by _sockfd as a passive socket,
    //  that is, as a socket that will be used to
...
void Server::initPoll(void) {
    ...
    _vectorPoll.push_back(
        _serverPoll); // set listen poll struct into the vector.
}
...
int Server::grabConnection() {
    ...
    if (fcntl(newClientFd, F_SETFL, O_NONBLOCK) < 0) {
        close(newClientFd);
        return (quickError("Error.\nNew connection could not be accepted.",
                           EXIT_SUCCESS));
    }
...
void Server::sendData(pollfd &pollStruct) {
    ...
    std::cout << "SendData, CMD recived:\n"
              << it->second->getRecvBuffer() << "**End of CLientbuffer.**"
              << std::endl;

    // return 1 if QUIT is send from client. Do not continue process.
    if (command.execute(*(it->second), *this))
        return;
    ...
    if (it->second->getLogStat()) {
        ...
        message.sendMessage(*it->second, ":Middleman 005 " +
